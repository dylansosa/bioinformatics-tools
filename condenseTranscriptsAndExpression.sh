...
# infile="$(cat test_Dmel_expression_noHeader)"
...
# for ((i = 4; i <=64; i++)); do awk -v column=$i '{ seen[$2] += $column } END { for (j in seen) print j, seen[j] }' test_Dmel_expression_noHeader | sort -k1 > Dmel_expression$i ; done

# gets just expression value column and saves in new files
#for ((k = 4; k <=64; k++)); do awk '{print $2}' Dmel_expression$k > expression$k ; done
